...
//  float vt_x= 0, vt_y= 0;
  ...
  static float v_array_x[TAKEOFF_POINT_NUMBER+1]=
  {PA_KT*0.86,-PA_KT*0.707,PA_KT,-PA_KT*0.94};
  static float v_array_y[TAKEOFF_POINT_NUMBER+1]=
  {PA_KT*0.5 ,PA_KT*0.707 ,0    , PA_KT*0.34};
  v_x= v_array_x[m_current_takeoff_point_id];
  v_y= v_array_y[m_current_takeoff_point_id];
  controlDroneVelocity(v_x, v_y , 0.0, 0.0);

  /*publish velocity*/
  /*geometry_msgs::Vector3Stamped velocity;
  velocity.header.frame_id= "to_setpoint_velocity";
  velocity.header.stamp= ros::Time::now();
  velocity.vector.x= v_x + vn_x;
  velocity.vector.y= v_y + vn_y;
