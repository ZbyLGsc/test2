        ...
        cv::Point2f& centerPoint)
{
    ...
    if( abs(D) < 1e-15) 
        return false;
    float C1 = radius1*radius1 - radius2*radius2 + x2*x2 - x1*x1 + y2*y2 - y1*y1;
    float C2 = radius2*radius2 - radius3*radius3 + x3*x3 - x2*x2 + y3*y3 - y2*y2;
        ...
        float& centerPoint_x, float& centerPoint_y)
{
    ...
    float move_vector_length = sqrt( pow(x4-centerPoint_x,2) + 
            pow(y4-centerPoint_y,2));
    if( abs(move_vector_length) < 1e-6)
        return false;
    float distance_to_correct = ( move_vector_length - radius ) / 2;
    centerPoint_x += distance_to_correct / move_vector_length
        * ( x4 - centerPoint_x); 
    centerPoint_y += distance_to_correct / move_vector_length
        * ( y4 - centerPoint_y); 
    return true;
}
// default constructor
...
void QRCode::getDetectionLocationAndDistance(vector< cv::Point2f >& detections_location,
        ...
        vector<AprilTags::TagDetection>& detections)
{
    ...
    for (int i=0; i<detections.size(); i++) {
        ...
        float ground_distance = sqrt( abs(pow(translation.norm(), 2) -
                    pow(detections_height, 2)) );
    ...
    if(detections_location.size()>=3)
    {
        ...
        for(int i=0; i<detections_location.size()-2; i++)
        {
            for(int j=i+1; j<detections_location.size()-1; j++)
            {
                ...
                {
                    if( calculateCenterPointFrom3Circles(detections_location[i],
                                detections_distance[i],
                                detections_location[j],
                                detections_distance[j],
                                detections_location[k],
                                detections_distance[k],
                                centerPoint) )
                    {
        ...
        //cout<<"corrected x:"<<centerPoint.x<<" y:"<<centerPoint.y<<endl;
...
/*void QRCode::loop(int argc, char* argv[]) {
    ...
    while (ros::ok()) {
        ...
        if( calculateBasePostion(detections_location,
                    detections_distance))
        {
            cout<<"base x:"<<base_position_x<<" y:"<<base_position_y<<endl;
        }
        else
        ...
        transform_test.setOrigin( tf::Vector3(translation(0), translation(1), translation(2)) );
        transform_test.setRotation( tf::Quaternion(0, 0, 0, 1) );
        br.sendTransform(tf::StampedTransform(transform_test, ros::Time::now(), "camera_rgb_optical_frame", "marker_frame"));
