...
Description:    提取指定颜色的区域k
Input:              src:     源图像
                color：选定的颜色
Output:             colorRegion: 提取的颜色区域
Others:
*************************************************/
void RMChallengeVision::extractColor(Mat src, COLOR_TYPE color,
                                     Mat& colorRegion)
{
  if(src.channels() != 3)
    return;
  ...
  static int iHighV;
  static int bgrThresh1; /*threshold of r-g g-r b-g ... depending on
                            the
                            color*/
...
Output:             realDistance:
物体实际到相机中心的水平距离，单位：mm
Others:             只有当已知物体实际尺寸时可用
*************************************************/
float RMChallengeVision::imageToRealDistance(float imageLength,
                                             float imageDistance,
                                             float realLength)
{
  float realDistance=
...
void RMChallengeVision::detectTriangle(Mat src, Mat color_region,
                                       int triangle[4])
{
...
/***************************************************************/
void RMChallengeVision::getYellowRegion(Mat& src, Mat& dst, int h_low,
                                        int h_high, int s_threshold,
                                        int v_threshold)
{
