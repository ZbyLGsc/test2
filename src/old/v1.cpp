...
// hero threshold
static int hLowH= 0;
static int hHighH= 100;
static int hLowS= 0;
static int hHighS= 255;
static int hLowV= 0;
static int hHighV= 255;
static int hThresh1= 25;
static int hThresh2= 25;
/************************************************************************************************/
...
/************************************************************************************************/
Mat drawCircle;
int color_signal= 1;  // 0 is red/else is blue
int taskFlag= 1;      // autonomous landing:0,following hero 1
              ...
              double &radius)
{
  if(src.channels() != 3)
    return;
  //        cvCreateTrackbar("LowH", "Control", &hLowH, 255); //Hue (0
  //        - 255)
  //        cvCreateTrackbar("HighH", "Control", &hHighH, 255);

  //        cvCreateTrackbar("LowS", "Control", &hLowS, 255);
  //        //Saturation (0
  //        - 255)
  //        cvCreateTrackbar("HighS", "Control", &hHighS, 255);

  //        cvCreateTrackbar("LowV", "Control", &hLowV, 255); //Value
  //        (0 -
  //        255)
  //        cvCreateTrackbar("HighV", "Control", &hHighV, 255);

  //        cvCreateTrackbar("br", "Control", &hThresh1, 255); //Value
  //        (0 -
  //        255)
  //        cvCreateTrackbar("bg", "Control", &hThresh2, 255);
  // to gray scale
  Mat gray;
  ...
  inRange(gray, Scalar(hLowH, hLowS, hLowV),
          Scalar(hHighH, hHighS, hHighV),
  ...
  Mat rlb, rmbabs, rb;
  cv::compare(bgrSplit.at(1), bgrSplit.at(2), rlb, CMP_GT);
  cv::absdiff(bgrSplit.at(1), bgrSplit.at(2), rmbabs);
  cv::threshold(rmbabs, rmbabs, hThresh1, 255, THRESH_BINARY);
  cv::bitwise_and(rlb, rmbabs, rb);
  // region where r>g and //region where |r-b|>t
  Mat rlg, rmgabs, rg;
  cv::compare(bgrSplit.at(1), bgrSplit.at(0), rlg, CMP_GT);
  cv::absdiff(bgrSplit.at(1), bgrSplit.at(0), rmgabs);
  cv::threshold(rmgabs, rmgabs, hThresh2, 255, THRESH_BINARY);
  ...
  Point pu(320, 0);
  Point pd(320, 480);
  detectLine(drawCircle, pl, pr, Scalar(0, 255, 0), 1);
  detectLine(drawCircle, pu, pd, Scalar(0, 255, 0), 1);
...
void findCircle(Mat src, Point2f &circle_center, double &rad)
{
  ...
  if(circle_center.x != 0 && circle_center.y != 0)
    detectLine(drawCircle, ccen, cen, Scalar(0, 0, 255), 2);
  ...
  for(int i= 0; i < contours.size(); i++)
  {
    ...
    cv::detectLine(drawCircle, end, start, dirColor, 2);
...
int main(int argc, char *argv[])
{
  ...
  VideoWriter write;
  string outFile= "/home/ubuntu/record1.avi";
  int w= 640;
  int h= 480;
  Size S(w, h);
