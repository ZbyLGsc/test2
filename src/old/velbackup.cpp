...
//    serial_port serialPort(iosev,"/dev/ttyTHS0");
    ...
    ros::Publisher color_pub = nh.advertise<std_msgs::String>("tpp/color", 10);

    ros::Subscriber rc_channels_sub = nh.subscribe("dji_sdk/rc_channels", 10,rcdataCallback);
    ros::Subscriber sub = nh.subscribe("tpp/real_distance", 1, getmessage);
    ros::spinOnce();
    ros::Rate r(100);
...
void missionUp()
{
    ...
    static double xy_threshold_high=0.35;
    ...
    //**************************************high height step*************************************//
    while(sqrt(pow(real_dis_x,2)+pow(real_dis_y,2))>xy_threshold_high||fabs(low_target_height-real_height)>height_threshold)
    {
//        ros::spinOnce();
